...
: ${1?"Usage: $0 -c clientname -s server -n subone -b subtwo -p pass -o outdir"}
...
#ccd部分
echo "ifconfig-push $SUBONE $SUBTWO">> /etc/openvpn/ccd/$CLIENTNAME
echo 'push "redirect-gateway def1 bypass-dhcp"' >>/etc/openvpn/ccd/$CLIENTNAME
echo "done"
