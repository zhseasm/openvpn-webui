...
if(checkEmpty($username,$password,$verifycode)){
    if(checkVerifycode($verifycode,$code)){
        if(checkUser($username,$password)){
            ...
            $flag=$mysqli->query("select user_id from user where user_name='{$username}'");
...
function checkUser($username,$password){
    $conn=new Mysql();
    $sql="select * from user where user_name='{$username}' and passwd=password('{$password}');";
    $result=$conn->sql($sql);
    if($result){
        return true;
    }
...
function checkEmail($email){
    ...
    if(preg_match($preg, $email)){
        ...
        echo '<html><head><Script Language="JavaScript">alert("邮箱格式有误");</Script></head></html>' . "<meta http-equiv=\"refresh\" content=\"0;url=dologin.php\">";
    }
}